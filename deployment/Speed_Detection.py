...
cap = cv2.VideoCapture(r'/Sample_Video/veh2.mp4')
    ...
    for bbox in bbox_id:
        ...
        if id in v_d:
            if cy2 < (cy+offset) and cy2 > (cy-offset):
                elps_time = time.time() - v_d[id]           ##time diff between time from line1 to line2
                if id not in cnt:
                    cnt.append(id)
                    distance = 10
        ...
        if id in v_u:
            if cy1 < (cy+offset) and cy1 > (cy-offset):
                elps_time = time.time() - v_u[id]           ##time diff between time from line1 to line2
                if id not in cnt:
                    cnt.append(id)
                    distance = 10
                    vech_speed = distance / elps_time
